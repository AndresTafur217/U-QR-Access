...
function mostrarAreas() {
  ...
  areas.forEach(area => {
    ...
    areaCard.innerHTML = `
      ...
      <div class="text-xs opacity-60 mb-5 leading-0.5">${area.descripcion}</div>
      <button class="w-full bg-sky-700 text-white py-3 px-6 border-none rounded-3xl cursor-pointer text-lg transition-all ease-in-out hover:scale-105" onclick="event.stopPropagation(); intentarIngresar(${area.id})">
          Ingresar
      </button>
    `;
